...
"""
	...
	def _set_stride(self, n):
		...
		self._freq = self._count / np.sum(self._count)
...
class TextDataset():
	...
	def __call__(self, **kwargs):
		...
		# if any of the keys are no present, values default_kwargs will not be overwritten
		
		default_kwargs = {'n':2, 'counts':False , 'common':False, 'most_frequent':None}
		self.mode = {**default_kwargs, **kwargs}			
		...
		if self.mode['common'] == False: global_index = reduce( np.union1d, [x._index for x in self._dataset] )
		...
		else: global_index = reduce( np.intersect1d, [x._index for x in self._dataset] )
